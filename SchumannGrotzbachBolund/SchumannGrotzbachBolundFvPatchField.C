...
#include "SchumannGrotzbachBolundFvPatchField.H"
...
SchumannGrotzbachBolundFvPatchField
(
    const SchumannGrotzbachBolundFvPatchField& ptf,
    const fvPatch& p,
    const DimensionedField<symmTensor, volMesh>& iF,
...
SchumannGrotzbachBolundFvPatchField
(
    const SchumannGrotzbachBolundFvPatchField& wfpf
)
:
...
void SchumannGrotzbachBolundFvPatchField::getRoughnessHeight(scalarField& z0_)
{
    // Face center height (z) field
    const vectorField& c = patch().Cf();
    label terrainFaceCount = 0;

    forAll(z0_, facei)
    {
        scalar surfaceHeight = c[facei] & vector(0., 0., 1.);
        // z0_[facei] = (surfaceHeight > refHeight())? z0Terrain_: z0Sea_;
        if(surfaceHeight > refHeight())
        {
            z0_[facei] = z0Terrain_;
            ++terrainFaceCount;
        }
        else
        {
...
void SchumannGrotzbachBolundFvPatchField::evaluate
(
...
vector SchumannGrotzbachBolundFvPatchField::transformVectorCartToLocal
(
    vector v, 
    vector xP, 
    vector yP, 
...
symmTensor SchumannGrotzbachBolundFvPatchField::transformSymmTensorLocalToCart
(
    symmTensor SP,
    vector xP,
    vector yP,
...
void  SchumannGrotzbachBolundFvPatchField::uStarEvaluate
(
    scalar& uStar, 
    scalar& L, 
    scalar& phiM, 
